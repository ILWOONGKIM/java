...
※ 『super』
	...
	static으로 선언되지 않은 메소드에서 사용되며						// check(클래스메소드에서 사용X)
	현재 클래스가 상속받은 상위 클래스의 객체를 가리킨다.
	super 는 상위 클래스의 생성자를 호출하거나
	상위 클래스의 멤버 변수 또는 메소드를 호출할 때 사용할 수 있다.

	하위 클래스의 생성자에서 상위 클래스의 생성자를 호출할 때에는		// 생성자 문법 中 하나
...
----------------------------------------------------------------------------
 ...
 않음				|	인수가 있는 생성자		|→	가능하다.(상위클래스 생성자를 주의할 필요 없음)
----------------------------------------------------------------------------
 인수가				|	생성자 정의 안함		|→	가능하다.
 없는				|	인수가 없는 생성자		|→	가능하다.
 생성자만 정의		|	인수가 있는 생성자		|→	가능하다.(상위클래스 생성자를 주의할 필요 없음)
----------------------------------------------------------------------------
 인수가				|	생성자 정의 안함		|→	에러 발생(*1)
 있는				|	인수가 없는 생성자		|→	상위 클래스의 해당 생성자를 호출하지 않으면 에러 발생.
 생성자만 정의		|	인수가 있는 생성자		|→	상위 클래스의 해당 생성자를 호출하지 않으면 에러 발생.
============================================================================

*1 : (디폴트 생성자가 자식클래스에 생성되고, 디폴트니까 인자없는 부모클래스 생성자의 구문이 자동으로 삽입되는데, 부모클래스 생성자에 인자가 있으므로 에러)
...
※	상속 시 주의할 사항
	...
	- Method Overloading 
			→ 아래에서 정리한 Method Overriding 개념과 구분하여 정리할 것
...
class Rect117 extends SuperTest117
{
	...
	public Rect117()
	{
		// 자동으로 삽입(부모클래스의 생성자 호출구문)
		//super();	→	SuperTest117();
	}
	

	public void clac(int w, int h)
	{
		this.w = w;
		this.h = h;
	...
	@Override				// 어노테이션(annotation)	→	metadata(; 기능에 영향을 미치지는 않으나 특정 데이터를 설명하기위한 데이터)(JDK 1.5)
							//								※자바의 컴파일과정에서 주석은 사라지나, metadata는 볼 수 있음

	public void write()		//	추가생성, 상속받은 메소드와 동일한 이름	정의 허용  →  Method Overloading
																					// 똑같은 이름으로 하나 더 만들기
	{
	...
	// ※ 메소드 오버라이딩(Method Overriding)	-	덮어쓰기, 상속일때만 적용
	//	  
...
// main() 메소드를 포함하고 있는 외부의 다른 클래스(but, 동일 패키지)
